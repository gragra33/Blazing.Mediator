...
public class PipelinePerformanceBenchmarks
{
    ...
    private void SetupMiddlewarePipelineBuilder()
    {
        ...
        for (int i = 0; i < 10; i++) _middlewarePipelineBuilder.AddMiddleware<FastTestMiddleware>();
    }
    ...
    private void SetupNotificationPipelineBuilder()
    {
        ...
        // Add more middleware to stress test the sorting algorithms
        for (int i = 0; i < 10; i++) _notificationPipelineBuilder.AddMiddleware<TestNotificationMiddleware>();
    }
    ...
    {
        return _middlewarePipelineBuilder!.AnalyzeMiddleware(_serviceProvider!);
    }
    ...
    public void MiddlewarePipeline_SortingPerformance()
    {
        ...
        IReadOnlyList<(Type Type, int Order, object? Configuration)> info =
            builder.GetDetailedMiddlewareInfo(_serviceProvider);
    }
...
public class PipelinePerformanceConfig : ManualConfig
{
    public PipelinePerformanceConfig()
    {
        ...
        if (OperatingSystem.IsWindows()) AddDiagnoser(new InliningDiagnoser(true, ["Blazing.Mediator"]));
...
public class FastTestMiddleware : IRequestMiddleware<TestRequest, string>, IRequestMiddleware<TestCommand>
{
    ...
    public async ValueTask HandleAsync(TestCommand request, RequestHandlerDelegate next,
    ...
    public async ValueTask<string> HandleAsync(TestRequest request, RequestHandlerDelegate<string> next,
        CancellationToken cancellationToken)
    {
        // Minimal processing to focus on pipeline performance
    ...
    where TRequest : IRequest
{
    ...
    public async ValueTask HandleAsync(TRequest request, RequestHandlerDelegate next,
        CancellationToken cancellationToken)
    {
        // Minimal processing to focus on pipeline performance
        await next().ConfigureAwait(false);
