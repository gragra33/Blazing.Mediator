...
public class NanosecondPrecisionBenchmarks
{
    ...
    {
        Type middlewareType = typeof(UltraFastMiddleware);
    ...
    {
        Type middlewareTypeDefinition = typeof(GenericUltraFastMiddleware<,>);
        Type[] fastFallbackTypes = new[] { typeof(SimpleRequest), typeof(object) };
        Type[] responseTypes = new[] { typeof(string), typeof(object) };
    ...
    {
        Type middlewareType = typeof(UltraFastMiddleware);
        Type[] interfaces = middlewareType.GetInterfaces();
        Type targetInterface = typeof(IRequestMiddleware<,>);

        // Optimized for loop instead of expensive LINQ
        for (int i = 0; i < interfaces.Length; i++)
        {
            Type iface = interfaces[i];
            if (iface.IsGenericType && iface.GetGenericTypeDefinition() == targetInterface) return true;
        }
    ...
    {
        Type genericTypeDefinition = typeof(GenericUltraFastMiddleware<,>);
        Type[] typeArguments = new[] { typeof(SimpleRequest), typeof(string) };

        // Simulate our optimized CanSatisfyGenericConstraints method
        Type[] genericParameters = genericTypeDefinition.GetGenericArguments();
        ...
        {
            Type parameter = genericParameters[i];
    ...
    {
        SimpleRequest request = new() { Data = "test" };

        ValueTask<string> FinalHandler()
        {
    ...
    {
        SimpleNotification notification = new() { Message = "test" };
        NotificationDelegate<SimpleNotification> finalHandler = (n, ct) => ValueTask.CompletedTask;
...
public class UltraFastMiddleware : IRequestMiddleware<SimpleRequest, string>
{
    ...
    public ValueTask<string> HandleAsync(SimpleRequest request, RequestHandlerDelegate<string> next,
