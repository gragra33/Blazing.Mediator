...
public class GenericQueryHandler : IRequestHandler<GenericQuery<List<int>>, string>
{
    ...
    public ValueTask<string> Handle(GenericQuery<List<int>> request, CancellationToken cancellationToken = default)
    {
        return ValueTask.FromResult($"Count: {request.Data?.Count ?? 0}");
    }
}