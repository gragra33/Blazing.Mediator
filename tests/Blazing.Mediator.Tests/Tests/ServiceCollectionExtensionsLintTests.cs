...
public class ServiceCollectionExtensionsLintTests
{
    ...
    public void ServiceCollectionExtensions_NullParameterHandling_DoesNotGenerateWarnings()
    {
        ...
        Should.NotThrow(() => services.AddMediator(null, null!));
        Should.NotThrow(() => services.AddMediator(null, (Type[])null!));
        Should.NotThrow(() => services.AddMediator(null, true, null!));
    ...
    public void ServiceCollectionExtensions_MiddlewareDiscoveryOverloads_HandleParametersCorrectly()
    {
        ...
        Should.NotThrow(() => services.AddMediator(false));
        Should.NotThrow(() => services.AddMediator(true));
        Should.NotThrow(() => services.AddMediator(false, typeof(ServiceCollectionExtensionsLintTests)));
        Should.NotThrow(() => services.AddMediatorWithNotificationMiddleware(false));
        Should.NotThrow(() => services.AddMediatorWithNotificationMiddleware(true));
    ...
    public void ServiceCollectionExtensions_DuplicateRegistrations_HandleGracefully()
    {
        ...
        Should.NotThrow(() =>
        {
            services.AddMediator(config => config.WithStatisticsTracking(), null!);
            services.AddMediator(config => { }, null!); // Should not override
    ...
    public void ServiceCollectionExtensions_ParameterValidation_WorksCorrectly()
    {
        ...
        Should.Throw<ArgumentNullException>(() => ((IServiceCollection)null!).AddMediatorWithNotificationMiddleware(false));
    }
}